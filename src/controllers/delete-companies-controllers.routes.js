...
deleteCompanies.delete('/', async (req, res) => {
  try {
    const { id } = validationId.parse(req.body)
    const companiesId = await pool.query('SELECT * FROM companies WHERE id = ?', [id])
    if (companiesId.length === 0) {
      return res.status(404).json({ message: 'id companies not font' })
    }
  
    const [data] = await pool.query('DELETE FROM companies WHERE id = ?', [id])
